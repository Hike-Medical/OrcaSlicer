...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->category = L("Quality");
    def->tooltip  = L("Enable Z-layer contouring (aka Z-layer anti-aliasing). Dynamically varies Z height within each layer to reduce stair-stepping on top surfaces.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));

    def = this->add("zaa_min_z", coFloat);
    def->label    = L("Z contouring minimum height");
    def->category = L("Quality");
    def->tooltip  = L("Minimum z layer height for contouring. Also controls slicing plane.");
    def->sidetext = L("mm");
    def->min      = 0;
    def->max      = 100;
    def->mode     = comAdvanced;
    ...
    def->category = L("Quality");
    def->tooltip  = L("When enabled, top surface infill direction is not alternated between layers. Useful for Z contouring.");
    def->mode     = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->category = L("Quality");
    def->tooltip  = L("Expand ironing area beyond the normal boundary. Useful with Z contouring.");
    def->sidetext = L("mm");
    def->min      = -100;
    def->max      = 100;
    def->mode     = comAdvanced;
