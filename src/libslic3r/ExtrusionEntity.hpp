...
public:
    ...
    float height;
    // ZAA: set to true if this path has been Z-contoured
    ...
    const Point3& first_point3() const { return this->polyline.points.front(); }
    const Point3& last_point3() const { return this->polyline.points.back(); }
    ...
    void   collect_points(Points &dst) const override;  // out-of-line: iterates Point3, emits Point
...
public:
    ...
    void reverse() override;
    Point first_point() const override { return this->paths.front().polyline.points.front().to_point(); }
    Point last_point() const override { return this->first_point(); }
...
inline void extrusion_entities_append_loops(ExtrusionEntitiesPtr &dst, Polygons &&loops, ExtrusionRole role, double mm3_per_mm, float width, float height)
{
    ...
    for (Polygon &poly : loops) {
        if (poly.is_valid()) {
            ...
            Polyline pl;
            pl.points = std::move(poly.points);
